...
function answer(chosen) {
  ...
  // How often SIRI has actually asked this, derived from the papers it
  // appeared on. No competitor scraping these PDFs surfaces this.
  const times = q.seen.length;
  const why = document.createElement('div');
  why.className = 'why';
  why.innerHTML = `
    <p class="lbl">${right ? 'Rigtigt' : 'Det rigtige svar'}</p>
    <p>${q.explain ?? `<b>${q.answer}</b>`}</p>
    <span class="src">
      ${times > 1
        ? `Stillet <b>${times} gange</b> siden 2020 (${q.seen.join(', ')}).`
        : `Stillet i <b>${q.seen[0].split('#')[0]}</b>.`}
...
async function main() {
  ...
  state.bank = questions.split('\n').filter(Boolean).map((line) => {
    ...
    return extra ? { ...q, explain: extra.explain, page: extra.page } : q;
  });
